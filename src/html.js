...
<body>
<header class="bar">
  <a class="word" href="/">Blinkboard</a>
...
<p>Pages are rented, public, and temporary. No free HTML. We refuse spam, phishing, malware, scams, and illegal content. Destination links must be https. URL shorteners, IP hosts, and credentialed URLs are blocked. Admin can expire or block a page immediately.</p></main>`,
    },
    terms: {
...
<p>No site accounts. We store your Telegram user id, page content, payment reference, and aggregate view counts. Images live in object storage until expiry. We do not sell personal data. Webhook traffic is authenticated with a secret token.</p></main>`,
    },
  };
  const p = pages[kind];
  if (!p) return null;
...
<p class="by"><a href="/">Blinkboard</a> · rented page · no account</p>
